...
func TestProcessNEM12File(t *testing.T) {
	tests := []struct {
		name          string
		input         string
		expectedLen   int
...
900`,
			expectError:  true,
			errorMessage: "invalid consumption value: strconv.ParseFloat: parsing \"INVALID\": invalid syntax",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			readings, err := ProcessNEM12File(reader)
			...
			if tt.expectError {
				...
				if len(readings) > 0 {
					...
					if firstReading.Nmi != tt.expectedNMI {
